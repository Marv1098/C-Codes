    ...
    switch (choice)
    {
        ...
        case 'd':
            ...
            cin >> Interest_Rate;
            {
                ...
                outfile << "Net worth after " << size / 12 << setw(33) << left << " years:" << setprecision(10) << Total
                        << endl;
                if ((size / 12) * 12 < size) {
                ...
                while (change_flag)
                {
                    cout << endl << "Do you need to change anything? (Y/N) ";
                    cin >> choice;

                    int month;
                    double *month_pointer = nullptr;

                    double amount;
                    if (choice == 'y' || choice == 'Y')
                    {
                        bool check = true;//checking if the input month is smaller than total amount of months
                        while (check) {
                            int M_index;
                            ...
                            if (month <= size)
                            {
                                ...
                                M_index = month - 1;
                                month_pointer = &saving[M_index];

                                cout << "Please enter the amount to change to: "; //changing current value to a new input value
                                cin >> amount;
                                *(month_pointer += M_index) = saving[M_index];
                                saving[M_index] = amount;

                                cout << "The amount for month " << month << " is now changed to $" << saving[M_index] << endl << "Following are updated yearly totals:" << endl;
